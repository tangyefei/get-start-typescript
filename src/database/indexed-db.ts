...
const IndexedDB = {
  ...
  openDB(dbname: string, objectStoreName: string, version: number, onSuccess: function) {
    var request: IDBOpenDBRequest = this.indexedDB.open(dbname, version || 1)
...
//         }
  ...
  // add(objectStoreName: string, argument: Note, callback: function) {
  //   if (db) {
//         // console.log(db, argument)
//         // 执行事务，添加数据到对象仓库（表）
//       var trans = db.transaction([objectStoreName], 'readwrite');
//       var request = trans.objectStore(objectStoreName)
//             .add(argument);
...
//   },
  ...
  searchAll: function(objectStoreName: string, callback: function/*, index, data*/) {
    if (db != null){
//         // console.log(db)
